...
export default function TeacherSidebar({ activeItem, user }: SidebarProps) {
  ...
  const menuItems = [
    ...
    { id: 'gider-pusulasi', label: 'Gider Pusulası', icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
  ...
  // Diamond icon SVG path
  const diamondIcon = 'M12 2L2 7l10 15 10-15L12 2zm0 2.8L19.6 7 12 18.5 4.4 7 12 4.8z';
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-64 bg-[#0F172A]/80 backdrop-blur-xl border-r border-[#D4AF37]/20 flex flex-col z-40">
      ...
      <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
        ...
        {/* Grup Derslerim - Premium özelliği */}
        {isPremium ? (
          <Link
            href="/teacher/group-classes"
            className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${
              activeItem === 'group-classes'
                ? 'bg-[#D4AF37]/10 text-[#D4AF37]'
                : 'text-slate-400 hover:bg-white/5 hover:text-white'
            }`}
          >
            ...
            <span className="font-medium">Grup Derslerim</span>
            {activeItem === 'group-classes' && (
              <span className="ml-auto w-1.5 h-1.5 bg-[#D4AF37] rounded-full" />
            )}
          </Link>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="font-medium text-slate-400 group-hover:text-slate-300 transition-colors">Grup Derslerim</span>
              <span className="ml-auto text-[9px] font-bold px-1.5 py-0.5 rounded bg-[#D4AF37] text-[#0F172A]">
                PRO
              </span>
            </div>
