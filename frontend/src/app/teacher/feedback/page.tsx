...
export default function TeacherFeedbackPage() {
  ...
  const loadReviews = async () => {
    try {
      ...
      const formattedReviews = reviewsData.map(r => ({
        ...
        student_name: studentMap.get(r.student_id) || 'Öğrenci',
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="font-serif text-2xl font-bold text-slate-900">Değerlendirmeler</h1>
        <p className="text-slate-600 mt-1">Öğrencilerinizden gelen yorumlar</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white/80 backdrop-blur-xl border border-white/50 rounded-3xl shadow-2xl shadow-[#0F172A]/5 p-6">
          <div className="flex items-center justify-between mb-4">
            <span className="text-sm font-medium text-slate-500">Toplam Değerlendirme</span>
            <div className="w-10 h-10 bg-[#D4AF37]/5 rounded-xl flex items-center justify-center">
      ...
      {reviews.length === 0 ? (
        <div className="bg-white/80 backdrop-blur-xl border border-white/50 rounded-3xl shadow-2xl shadow-[#0F172A]/5 p-12 text-center">
          ...
          <p className="text-slate-600">Henüz değerlendirme bulunmuyor.</p>
          <p className="text-sm text-slate-400 mt-2">Derslerinizi tamamladıktan sonra öğrencileriniz yorum yapabilir.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {reviews.map((review) => (
            <div key={review.id} className="bg-white/80 backdrop-blur-xl border border-white/50 rounded-3xl shadow-2xl shadow-[#0F172A]/5 p-6">
