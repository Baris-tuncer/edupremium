  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Hakediş Yönetimi</h1>
      ...
      <div className="bg-[#D4AF37]/5 border border-[#D4AF37]/20 rounded-xl p-4 mb-6">
        <h4 className="font-semibold text-[#0F172A] mb-2">Hakediş Sistemi</h4>
        <div className="text-sm text-[#D4AF37] space-y-1">
        ...
        <table className="w-full">
          <thead className="bg-slate-50 border-b">
            <tr>
              ...
              <th className="px-6 py-4 text-right text-sm font-semibold text-slate-900">Saatlik Ücret</th>
              <th className="px-6 py-4 text-right text-sm font-semibold text-slate-900">Tamamlanan Ders</th>
              <th className="px-6 py-4 text-right text-sm font-semibold text-slate-900">Bekleyen</th>
              <th className="px-6 py-4 text-right text-sm font-semibold text-slate-900">Çekilebilir</th>
              <th className="px-6 py-4 text-right text-sm font-semibold text-slate-900">Toplam Kazanç</th>
                ...
                <tr key={teacher.id} className="hover:bg-slate-50">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-[#D4AF37]/10 rounded-full flex items-center justify-center font-semibold text-[#D4AF37]">
      ...
      {selectedTeacher && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl max-w-lg w-full">
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-slate-900">{selectedTeacher.email}</p>
                </div>

                <div>
                  <label className="text-sm font-medium text-slate-500">IBAN</label>
                  <p className="text-slate-900 font-mono">{selectedTeacher.iban || 'Girilmemiş'}</p>
                </div>

                <div className="bg-slate-50 p-4 rounded-lg">
                  <h4 className="font-semibold text-slate-900 mb-3">Cüzdan Detayları</h4>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-slate-500">Bekleyen Bakiye:</span>
                      <span className="font-medium text-yellow-600">
                        ₺{(selectedTeacher.wallet?.pendingBalance || 0).toLocaleString('tr-TR')}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Çekilebilir Bakiye:</span>
                    ...
                    <div className="flex justify-between">
                      <span className="text-slate-500">Toplam Çekilen:</span>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium text-slate-500">Saatlik Ücret</label>
                    <p className="text-slate-900">₺{selectedTeacher.hourlyRate.toLocaleString('tr-TR')}</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-slate-500">Tamamlanan Ders</label>
                    <p className="text-slate-900">{selectedTeacher.completedLessons || 0}</p>
                  </div>
                </div>
              </div>
