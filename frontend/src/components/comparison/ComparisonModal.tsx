  ...
  const comparisonRows = [
    {
      ...
      getValue: (t: ComparisonTeacher) => {
        return t.hourly_rate > 0 ? `${t.hourly_rate.toLocaleString('tr-TR')} TL` : '-';
      },
    },
    {
      label: 'Deneyim',
      icon: <Clock className="w-5 h-5" />,
      getValue: (t: ComparisonTeacher) => t.experience_years ? `${t.experience_years} yıl` : '-',
    },
    {
      label: 'Puan',
      icon: <Star className="w-5 h-5" />,
      getValue: (t: ComparisonTeacher) => t.rating ? `${t.rating.toFixed(1)} / 5` : '-',
    },
    {
      label: 'Üniversite',
      icon: <GraduationCap className="w-5 h-5" />,
      getValue: (t: ComparisonTeacher) => t.university || '-',
    },
    {
      label: 'Dersler',
    ...
    },
    {
      ...
      getValue: (t: ComparisonTeacher) => t.title || '-',
    },
  ];
          ...
          >
            ...
            {selectedTeachers.map(teacher => (
              <div key={teacher.id} className="text-center">
                ...
                <h3 className="font-bold text-[#0F172A] mb-1">{teacher.name}</h3>
                <Link
