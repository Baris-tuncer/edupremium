...
export default function Sidebar() {
  ...
  return (
    <aside className="w-72 bg-[#0F172A]/80 backdrop-blur-xl text-slate-300 min-h-screen flex flex-col fixed left-0 top-0 z-30 border-r border-[#D4AF37]/20">
      <div className="p-8 pb-4">
        <div className="flex items-center gap-3 mb-8">
           <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center text-white font-bold text-xl">E</div>
           <h1 className="font-bold text-2xl text-white tracking-tight font-serif">EduPremium</h1>
        </div>
      ...
      <nav className="flex-1 px-4 space-y-2">
        {menuItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className={`flex items-center gap-4 px-4 py-4 rounded-xl transition-all duration-200 group ${isActive ? 'bg-[#D4AF37]/10 text-[#D4AF37]' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}>
              <span className={`text-xl transition-transform group-hover:scale-110 ${isActive ? 'text-[#D4AF37]' : 'text-slate-400 group-hover:text-white'}`}>{item.icon}</span>
              <span className="font-medium">{item.label}</span>
              {isActive && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-[#D4AF37]"></div>}
            </Link>
          )
        })}
      </nav>
